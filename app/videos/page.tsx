      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="mb-16">
          <h1 className="text-5xl md:text-8xl font-black uppercase tracking-tighter leading-none mb-8">
            Our <span className="text-green-600">Videos</span>
          </h1>
              ...
              return (
                ...
                >
                  <AnimatePresence mode="wait">
                    {isPlaying ? (
                      <motion.div key="player" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black">
                        <button 
                        ...
                        <iframe
                          ...
                          allowFullScreen
                          style={video.platform === 'instagram' ? { height: '115%', transform: 'scale(1.02)', transformOrigin: 'top' } : {}}
                        />
                      </motion.div>
                    ) : (
                      <motion.div
                      ...
                      >
                        {/* Branded Fallback */}
                        <div className={`absolute inset-0 flex items-center justify-center bg-linear-to-br ${
                          video.platform === 'youtube' ? 'from-red-600 to-red-800' : 'from-[#f09433] via-[#dc2743] to-[#bc1888]'
                        }`}>
                          {video.platform === 'youtube' ? <FiYoutube size={80} className="text-white opacity-20" /> : <FiInstagram size={80} className="text-white opacity-20" />}
                        </div>
                        ...
                        <div className="absolute bottom-0 left-0 p-8 z-30 w-full">
                          <div className="flex items-center justify-between mb-3">
                            <div className="flex items-center gap-2">
                              {video.platform === 'youtube' ? <FiYoutube className="text-red-500" /> : <FiInstagram className="text-pink-500" />}
        ...
        {filteredVideos.length === 0 && (
          <div className="py-24 text-center">
            <h3 className="text-2xl font-black uppercase text-slate-300">No {activePlatform} videos found.</h3>
            <button 
